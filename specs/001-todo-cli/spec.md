...
- 기본 로컬 저장소는 파일 기반(예: JSON 또는 소형 로컬 DB)으로 가정하나, 기술 스택은 미정이며 설계 단계에서 결정한다.
 - 기본 로컬 저장소는 SQLite 단일 파일 DB(예: ~/.todo-cli/todo.db)로 가정한다. 성능·무결성(트랜잭션)을 제공하므로 로컬 동시성 처리가 필요한 시나리오에 적합하다.
 - 기술 스택은 일부 구현 세부를 제외하고는 미정이며 설계 단계에서 추가 결정이 가능하다.
